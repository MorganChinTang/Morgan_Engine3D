...
void GameState::Initialize()
{
    ...
    for (uint32_t i = 0; i < numSegments; i++)
    {
        ...
        Segments& part = mSegments.emplace_back();
        part.segment.meshBuffer.Initialize(sphere);
        part.segment.diffuseMapId = tm->LoadTexture("Images/cyan.jpg");
...
void GameState::Terminate()
{
    ...
    for (Segments& segment : mSegments)
    {
...
{
    // start at 1 becuase 0 is managed by physics
    // also starting at 1 means that previous value is accessible
    for (uint32_t i = 1; i < mSegments.size(); i++)
    {
        Segments& prevSeg = mSegments[i - 1];
        Segments& seg = mSegments[i];
...
void GameState::DebugUI()
{
    ...
    Math::Vector3 pos = mGroundObject.transform.position;
    if (ImGui::DragFloat3("BallPosition", &pos.x))
    {
        mGroundObject.transform.position = pos;
        mGroundRigidBody.SetPosition(mGroundObject.transform.position);
    }
