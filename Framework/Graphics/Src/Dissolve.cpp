...
{
	ASSERT(mCamera != nullptr, "DissolveEffect: No camera specified.");
	ASSERT(mDirectionalLight != nullptr, "DissolveEffect: No light specified.");
...
void DissolveEffect::Render(const RenderGroup& renderGroup)
{
	ASSERT(mCamera != nullptr, "DissolveEffect: No camera specified.");
	ASSERT(mDirectionalLight != nullptr, "DissolveEffect: No light specified.");

	Math::Matrix4 matWorld = renderGroup.transform.GetMatrix4();
	Math::Matrix4 matView = mCamera->GetViewMatrix();
	Math::Matrix4 matProj = mCamera->GetProjectionMatrix();
...
{
	if (ImGui::CollapsingHeader("DissolveEffect", ImGuiTreeNodeFlags_DefaultOpen))
	{
		bool useShadowMap = mSettingsData.useShadowMap > 0;
		if (ImGui::Checkbox("UseShadowMap##DissolveEffect", &useShadowMap))
		{
			mSettingsData.useShadowMap = useShadowMap ? 1 : 0;
		}
		ImGui::DragFloat("DepthBias##DissolveEffect", &mSettingsData.depthBias, 0.000001f, 0.0f, 1.0f, "%.6f");
		ImGui::DragFloat("LowHeight##DissolveEffect", &mSettingsData.lowHeight, 0.001f, 0.0f, 20.0f);
		ImGui::DragFloat("BlendHeight##DissolveEffect", &mSettingsData.blendHeight, 0.001f, 0.0f, 20.0f);
	}
}
