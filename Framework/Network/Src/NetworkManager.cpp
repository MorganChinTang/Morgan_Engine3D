...
void NetworkManager::Update(float deltaTime)
{
    ...
    if (!mObtainLatency)
    {
        ...
        if (mPlayerIds.size() > 1)
        {
            ...
            if (itr != mNetworkControllers.end())
            {
                ...
                {
                    /*char msgData[100];
                    sprintf_s(msgData, "%d %s %d %d %d %d",
                        (int)EventType::Input, mPlayerIds[0].c_str(), input.moveX, input.moveY, input.jump, input.shiftSpeed);
                    SendMsg(msgData, 100);*/
                }
    ...
    case EventType::SetId:
    {
        ...
        sscanf_s(data, "%d %s %s",
            &eventType,
            senderId, (unsigned int)sizeof(senderId),
            localId, (unsigned int)sizeof(localId));
        ...
        char msgData[100];
        sprintf_s(msgData, "%d %s", (int)EventType::IdReceived, GetLocalId().c_str());
        SendMsg(msgData, 100);

    }
    break;
    //only done by server
    case EventType::IdReceived:
    {
        int eventType = 0;
        char senderId[100];

        sscanf_s(data, "%d %s", &eventType, senderId, (unsigned int)sizeof(senderId));
    ...
    case EventType::Update:
    {
        ...
        if (itr != mNetworkControllers.end())
        {
            ...
            targetTrans.position = pos + vel * (2.0f * mLatencyAverage);
