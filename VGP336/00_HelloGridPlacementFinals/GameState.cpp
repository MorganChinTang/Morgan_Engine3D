...
namespace
{
    void SpawnFurniture(GameWorld& gameWorld, GridPlacementController& gridPlacementController)
    {
        auto SpawnAt = [&gameWorld, &gridPlacementController](const char* name, const wchar_t* templatePath, const Vector3& position)
            {
                GameObject* go = gameWorld.CreateGameObject(name, templatePath);
                if (go == nullptr)
                {
                    return;
                }
                ...
                go->Initialize();
                gridPlacementController.RegisterPickableFurniture(*go);
            };

        SpawnAt(
            "Furniture_Cactus_01", L"../../Assets/Templates/Objects/furniture_cactus.json",
            { -3.0f, 0.0f, 2.0f });
        SpawnAt(
            "Furniture_Cactus_02", L"../../Assets/Templates/Objects/furniture_cactus.json",
            { -1.0f, 0.0f, 2.0f });
        SpawnAt(
            "Furniture_Salad_01", L"../../Assets/Templates/Objects/furniture_salad.json",
            { 1.0f, 0.0f, 2.0f });
        SpawnAt(
            "Furniture_Salad_02", L"../../Assets/Templates/Objects/furniture_salad.json",
            { 3.0f, 0.0f, 2.0f });
    }
}
...
{
    mLevelFile = L"../../Assets/Templates/Levels/grid_placement_level.json";
    ...
    SpawnFurniture(mGameWorld, mGridPlacementController);
