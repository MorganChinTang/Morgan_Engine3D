...
void GridPlacementController::UnregisterPickableFurniture(GameObject& gameObject)
{
    mPickableFurniture.erase(
        std::remove(mPickableFurniture.begin(), mPickableFurniture.end(), &gameObject),
        mPickableFurniture.end());

    mLockedPlacements.erase(&gameObject);

    if (mHeldState.gameObject == &gameObject)
    {
        mHeldState = {};
    }
}

bool GridPlacementController::IsHoldingObject(const GameObject* gameObject) const
{
    return mHeldState.gameObject != nullptr && mHeldState.gameObject == gameObject;
}
